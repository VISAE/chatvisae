...
//$subject = getstring("mail.user.history.subject", true)." [".$threadid."]";//original
$subject = "Copia de conversación [".$threadid."]. Asesoría Virtual VISAE-UNAD";
$body = getstring2("mail.user.history.body", array($thread['userName'],$history), true);


$link = connect();
//mibew_mail($email, $mibew_mailbox, $subject, $body, $link);//original
mibew_mail_gmail($email, $mibew_mailbox, $subject, $body, $link);
mysql_close($link);
