...
function setup_chatview_for_operator($thread, $operator)
{
	...
	$page['asignarTemaLink'] = safe_htmlspecialchars("$mibewroot/modulo/tema/form_asignacion.php?" . $params);
	$page['notificarValoracionLink'] = safe_htmlspecialchars("$mibewroot/modulo/valoracion/form_notificar_valoracion.php?" . $params . "&email=" . $thread['email_user']);
}
...
function close_thread($thread, $isuser)
{
	...
	$link = connect();
	//------------------------------------------------------------------	
	notificar_valoracion($thread['threadid'],$thread['ltoken'],$thread['email_user'],$link);	
	//------------------------------------------------------------------
...
function thread_by_id_($id, $link)
{
	...
	return select_one_row("select threadid,userName,agentName,agentId,lrevision,istate,ltoken,userTyping,agentTyping" .
						  ...
						  ",remote,referer,locale,unix_timestamp(lastpinguser) as lpuser,unix_timestamp(lastpingagent) as lpagent, unix_timestamp(CURRENT_TIMESTAMP) as current,nextagent,shownmessageid,userid,userAgent,groupid,email_user,valoracion_notificada" .
...
function actualizar_email_usuario($thread,$email,$link)
{
	global $mysqlprefix;
	$query = sprintf("update ${mysqlprefix}chatthread set email_user = '%s' " .
			"where threadid = %s ",
