...
function mibew_mail_gmail($toaddr, $reply_to, $subject, $body, $link)
{
	global $mibew_encoding, $mibew_mailbox, $mail_encoding, $current_locale;
    ...
    $mail->Username="alertas.consejeria@unad.edu.co";// SMTP usuario
    $mail->Password="C0nsejer1a+*";// SMTP clave
    $mail->From="no-reply@unad.edu.co";// Mail Remite
    $mail->FromName="UNAD - Auditoría de Servicios a Estudiantes VISAE";// nombre remite
    $mail->IsHTML(true);// enviar como HTML
    $mail->AddAddress($toaddr,$toaddr);// direccion y nombre del destino
    $mail->WordWrap=70;
    $mail->Subject=$subject;// asunto del e-mail
    $mail->Body=$body;// cuerpo del e-mail en html
    //$mail->AltBody=$mensajetxt;// cuerpo alternativo en texto
	if(!$mail->Send()) {
  		$envio= "Mailer Error: " . $mail->ErrorInfo;
	} else {
 	 	$envio= "Message sent!";
	}
	$mail->ClearAddresses(); 
