...
if( !isset($_GET['token']) || !isset($_GET['thread']) ) {
	...
	if( !$thread ) {
		...
		if($email) {
			actualizar_email_usuario($thread['threadid'],$email,$link);
...
if($thread['istate']=="3"){//si se trata de acceder a la conversación cuando ya está cerrada, se lanza a la pagina de inicio
	header("Location: $mibewroot/client.php?level=$level".($chatstyle ? "&style=$chatstyle" : ""));
	exit;
}
